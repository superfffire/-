...
r_int2=6.5e-3;%hubradius(m,N2,h_a_star,c_star);
%% Enter Crack Values
q_0=0;
alfa_c=0*pi/180;
...
if length(betai) == length(K) && length(K) == length(K_M)
    data = table(betai,K,K_M,'VariableNames',{'betai','K','K_M'});

    filename ='K_betai_K_M.csv';
